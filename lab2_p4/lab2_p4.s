 ...
 *
 * Description: Controlling 8 LEDs with a button to inplement a shift pattern.
 *
 */
...
main:
	...
	/* setup PB1 for all LED  for bits 0-15 in MODER */
	ldr r6, =GPIOB_MODER
	ldr r5, [r6]
	...
	/* setup PB0-6-7 for LED  for bits 0-6-7 in ODR */
	ldr r6, =GPIOB_ODR
	ldr r5, [r6]
	/* cannot do with movs, so use pc relative */
	ldr r4, =0xFFFF
	...
	/* setup PB0-1-7 for LED  for bits 0-6-7 in ODR */
	ldr r6, =GPIOB_ODR
	ldr r5, [r6]
	/* cannot do with movs, so use pc relative */
	ldr r4, =0xFFFF
